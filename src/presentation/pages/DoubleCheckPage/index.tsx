...
const DoubleCheckPage = () => {
    ...
    const getValue = useCallback(()=>{
        ...
        data?.checkListDtoList?.forEach((checklist: any) => {
            count += checklist.itemDtoList.filter((item: any) =>
            item.isChecked === true
        ).length;
        });
    ...
    */
    ...
    return (
        ...
        <Container>
            <BackHeaderWrapper>
                <div></div>
                <Text text="더블 체크" color={COLOR.GRAY_900} fontSize={18} lineHeight="25.2px" fontWeight={700} />
                <Icon icon="CloseOutlined" fill={COLOR.MAIN_BLACK} onClick={onClickExit} />
            </BackHeaderWrapper>
            <ProgressBar  max={getMax()} value={getValue()} percent={false} size={'small'} startColor={COLOR.MAIN_GREEN}/>
            <Spacing size={53.15}/>
            

            {data?.checkListDtoList?.map((checklisttmp: any, index: any) => (
                checklisttmp.title !== "" &&
                <>
                <Title>
                    ...
                    <TitleLeft>
                        <CheckListIndicator 
                        checked={checklisttmp?.itemDtoList?.filter((item : any) => item.isChecked === true).length} 
