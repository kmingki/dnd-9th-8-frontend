...
import {
  ...
  Droppable,
  DropResult,
} from "react-beautiful-dnd"; //drag and drop
...
const AddCheckList = ({
  ...
  onChangeCheckItem,
  onClickPlusItem,
  onChangeCheckItemTitle,
  onClickDeleteCheckItem,
}: checkListType) => {
  const { tripId } = useParams();
  const [checklisttitle, setTitle] = useState(title);
  const [itemlist, setItemlist] = useState<listItem[]>(itemDtoList);
  const [isOpen, setIsOpen] = useState(false);
  const { mutate: updateChecklistMutate /*data , isLoading, error*/ } =
    useUpdateChecklist();
  const { mutate: changeOrderItemMutate /*data , isLoading, error*/ } =
    useChangeOrderItem();
  ...
  const onDragEnd = ({ source, destination, draggableId }: DropResult) => {
    ...
    const newItemsTmp = newItems.map((i: listItem, index: number) => {
      return { id: i.itemId, order: index + 1 };
    });
        ...
        <Head>
          <Title>
            ...
            <TitleLeft>
              <Indicator>
                <span>
                  {itemDtoList.filter((item) => item.isChecked === true).length}
                </span>
                /{itemDtoList.length}
              </Indicator>
              {isOpen ? (
